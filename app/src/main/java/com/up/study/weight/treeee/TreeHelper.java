...
public class TreeHelper {
	...
	public static <T> List<Node> convertData2Nodes(List<T> datas, boolean isHide)
		...
		for (T t : datas) {
			...
			for (Field f : declaredFields) {
				...
				if (id == -1 && pId == -1 && name == null&&type==-1) {
					break;
				}
			}

			node = new Node(id, pId, name,type);
		...
		for (int i = 0; i < nodes.size(); i++) {
			...
			for (int j = i + 1; j < nodes.size(); j++) {
				Node m = nodes.get(j);
				if (n.getId() == m.getpId() && (m.getType()-n.getType()==1)) {
					n.getChildrenNodes().add(m);
					m.setParent(n);
				} else if (n.getpId() == m.getId()&&(n.getType()-m.getType()==1)) {
					n.setParent(m);
					m.getChildrenNodes().add(n);
				}

				//大章节下可以直接挂知识点
				if (n.getId() == m.getpId() && m.getType()==3&&n.getType()==1) {
					n.getChildrenNodes().add(m);
					m.setParent(n);
				} else if (n.getpId() == m.getId()&&m.getType()==1&&n.getType()==3) {
					n.setParent(m);
					m.getChildrenNodes().add(n);
				}
			}
	...
	public static void setNodeIcon(Node node) {
		if (node.getChildrenNodes().size() > 0 && node.isExpand()) {
			node.setIcon(R.mipmap.ct_10);
		} else if (node.getChildrenNodes().size() > 0 && !node.isExpand()) {
			node.setIcon(R.mipmap.ct_11);
		} else
			node.setIcon(-1);
	}
